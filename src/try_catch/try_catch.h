...
{
    FilePrintf( FILE *f = stdout ) : mFile( f )
    {
    }
...
struct thread_try_points_base
{
    ...
    thread_try_points_base( try_point_data *p = nullptr, size_t cap = 0 ) : mTryPoints( p ), mCap( cap )
    {
    }
...
struct thread_try_points : public thread_try_points_base_t
{
    ...
    thread_try_points() : thread_try_points_base()
    {
        mTryPoints = &mData[0];
        mCap = N;
        mSize = 0;
...
{
    const bool bPop;
    _AutoTryPointPop() : bPop( _is_try_catch_installed() && !_get_thread_try_points().full() )
    {
    }
...
{
    try_point_data d;
    TryPointRestore( const try_point_data &a ) : d( a )
    {
    }
    ~TryPointRestore()
    {
