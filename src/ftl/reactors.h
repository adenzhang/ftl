...
struct IEventSender: public IEventSenderBase
{
    ...
    virtual bool Send( const EventType& ) = 0;
...
//    virtual IEventSender<EventType>* RegisterHandler( const std::function< void (const EventType&) >& handler, size_t eventsCapacity ) = 0;
    ...
    virtual void TryPush( const std::function<void()>& ) {}
...
public:
    ...
    PooledEventSender(ReactorsBasePtr d, const Handler& h, size_t siz )
...
protected:
    ...
    using TaskQ = blocking_queue< Task >;
...
//        std::cout << " joining threads\n";
    ...
    template<typename EventType>
    PooledEventSender<EventType>* RegisterHandler( const std::function< void (const EventType&) >& handler, size_t eventsCapacity = 256 )
    {
    ...
    void TryPush( const std::function<void()>& task ) override {
...
protected:
    ...
    lockfree_queue<EventType> events;  // lockfree SPSC queue
#else
    blocking_queue<EventType> events;  // SPSC queue
#endif
#endif
...
public:
    ...
    SeparatedEventSender(const Handler& h, size_t siz )
        : handler(h), events(siz), stopping(false)
    {
...
public:
    ...
    template<typename EventType>
    SeparatedEventSender<EventType>* RegisterHandler( const std::function< void (const EventType&) >& handler, size_t eventsCapacity = 32 )
    {
        if( _stopping ) return nullptr;
        std::lock_guard<std::mutex> guard( _handlersMutex );
...
#else
#ifdef USE_LOCKFREE_QUEUE
    using QueueType = lockfree_queue<Task>; // lockfree MPSC queue
#else
    using QueueType = blocking_queue<Task>;  // MPSC queue
#endif
#endif
...
public:
    ...
    // should not be called directly, use PooledEventDispatcher::RegisterHandler() to create it.
    GroupedEventSender( const Handler& h, QueueType& events )
...
#ifdef QUEUE_TBB
        try{
            events.push( [e, this]{ handler( e ); --currToken; } );
        }catch( tbb::user_abort& ){
            return false;
        }

#else
        while( ! events.try_push( [e, this]{ handler( e ); --currToken; }, 1000000 ) )  {
#ifdef _REACTOR_DBG
            ss << name << " failed to push event " << e ;
            std::cout << ss.str()<< std::endl;
            ss.str("");
...
protected:
    ...
    using Task = std::function<void()>;
#ifdef USE_LOCKFREE_QUEUE
    using TaskQ = lockfree_queue< Task >; // lockfree MPSC queue
#else
...
//            std::cout << "clearing threads" <<  std::endl;
    ...
    template<typename EventType>
    GroupedEventSender<EventType>* RegisterHandler( const std::function< void (const EventType&) >& handler, size_t )
    {
        if( _stopping ) return nullptr;
        std::lock_guard<std::mutex> guard( _handlersMutex );
        ...
        for(size_t i=0; i< _tasksQ.size(); ++i) {
            if( _handlers[i].size() < _handlers[k].size() ) k = i;
        }
