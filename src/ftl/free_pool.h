...
struct GenDeleter
{
    ...
    template<class DT>
    GenDeleter( DelFn pF, DT pD ) : pDelFn( pF ), pDeleter( static_cast<D>( pD ) )
    {
    }
...
template<class T,
         class D = IDeallocator<T>,
         class SaveOrRefDeallocTag = RefDeallocator_Tag,
         member_deallocate_pointer<D, T> DelFn = static_cast<member_deallocate_pointer<D, T>>( &D::deallocate )>
...
template<class T,
         class D = IDeleter<T>,
         class CopyOrRef = CopyDeallocator_Tag,
         member_deallocate_pointer<D, T> DelFn = static_cast<member_deallocate_pointer<D, T>>( &D::operator() )>
...
public:
    ...
    void deallocate( T *p, size_t n )
    {
        ...
        assert( p );
        if ( n != 1 )
            throw std::runtime_error( "poll only allocate/deallocate one object" );
        freeList.push( *p );
    }
...
template<typename T,
         ...
         class FreeList = FreeList<T, null_delete<T>>>
...
public:
    ...
    // pre-allocate memory. memory will be deleted when FreePool destructs.
    // T must be IDeallocatable
    PoolAllocator( const Alloc &a = Alloc(), const FreeList &freeList = FreeList() ) : parent_type( freeList ), mAlloc( a )
    {
    }
