...
struct IDeletable : IDeletableBase<IDeletable<T, D, SaveOrRefDeallocTag>, T, D, SaveOrRefDeallocTag> {
    ...
    IDeletable(deallocator_type& dealloc)
        : parent_type(dealloc)
    {
    }
...
struct FreeList {
    struct FreeNode {
        ...
        FreeNode(T* next = nullptr)
            : pNext(reinterpret_cast<FreeNode*>(next))
        {
        }
        FreeNode(FreeNode* next = nullptr)
            : pNext(next)
        {
        }
    };

    FreeList(T* p = nullptr, const Deleter& d = {})
        : pHead(reinterpret_cast<FreeNode*>(p))
        , mDel(d)
    {
    }

    FreeList(const Deleter& d)
        : mDel(d)
    {
    }
...
struct ConstGrowthStrategy {
    ConstGrowthStrategy(size_t initial)
        : value(initial)
    {
    }
    size_t next_value()
...
struct DoubleGrowthStrategy {
    DoubleGrowthStrategy(size_t initial)
        : value(initial)
    {
    }
    size_t next_value()
