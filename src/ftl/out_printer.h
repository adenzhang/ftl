...
struct OutPrinter {
    ...
    template <class T>
    static constexpr bool is_specially_handled = IsIterable<T>::value || is_pair<T>::value || is_tuple<T>::value || is_optional<T>::value || has_c_str<T>::value || is_like_ptr<T>::value;

    OutPrinter(std::ostream& o = std::cout)
        : os(o)
    {
    }
