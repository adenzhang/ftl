...
struct Node : Deletable<Node> // IDeallocator::operator()(T*)
{
...
typedef ManagedChunkPool<Node, ConstGrowthStrategy> NodePool;
#else
...
static int main_test( int argc, const char *argv[] )
{
    ...
    //    Apr apr;
    int min_depth = 4;
    int max_depth = std::max( min_depth, ( argc == 2 ? atoi( argv[1] ) : 10 ) );
    int stretch_depth = max_depth + 1;

    //    assert( stretch_depth < 0);
    assert( stretch_depth < 32 );
    const size_t MaxNodes = CalcNodes( stretch_depth ); // nodes: 2**(depth+1)-1
    //    std::cout << "allocated :" << MemSize << " nodes:" << MaxNodes << " blocksize:" << sizeof(Node) << " stretch_depth:" << stretch_depth <<
    //    std::endl;
...
ADD_TEST_FUNC( managed_object_pool_tests )
{
    REQUIRE( 1 <= 3 );
    const char *argv[] = {"main", "5"};
    std::cout << "managed_object_pool_tests starting..." << std::endl;
    main_test( 2, argv );
}
